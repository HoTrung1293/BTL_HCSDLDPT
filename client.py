...
menu = ["Trích Xuất File Âm Thanh","Tìm File Âm Thanh Tương Đồng",  "Về Ứng Dụng"]
...
if choice == "Tìm File Âm Thanh Tương Đồng":
    ...
    # Lấy danh sách file âm thanh từ thư mục data
    ...
    if selected_file:
        ...
        if nearest:
            ...
            # Đếm số lượng file output thực tế (trừ đi file input)
            output_count = len([f for f, _, _ in nearest if f != selected_file])
            # Tạo hình vẽ với số lượng subplots phù hợp (input + các file output)
            fig, axes = plt.subplots(1, output_count + 1, figsize=(20, 4))
            
            # Đảm bảo axes luôn là một mảng ngay cả khi chỉ có 1 subplot
            ...
            # Tạo dataframe so sánh từ nearest và input_feats
            ...
            # Thêm dữ liệu từ nearest matches
            for idx, (fname, dist, feats) in enumerate(nearest):
                if feats is not None:  # Check if feats exist
            ...
            # Make sure there are features to select
            available_features = [f for f in feature_names if f in df_compare.columns]
            default_features = ["pitch", "energy", "zcr", "centroid", "bandwidth", "silence_ratio"]
            ...
            selected_features = st.multiselect("Chọn đặc trưng để so sánh", 
                                              available_features, 
                                              default=default_features[:4])  # Limit to 4 defaults
            ...
            if selected_features and len(df_compare) > 0:
                try:
                    ...
                    # Biểu đồ radar cho các đặc trưng
            ...
            # Biểu đồ nhiệt so sánh các file
            if len(df_compare) > 0:
                st.subheader("Heat Map")
                # Get available features that are actually in the dataframe
...
# CHỨC NĂNG 2: TRÍCH XUẤT FILE ÂM THANH
    ...
    if extract_option == "Trích xuất thư mục vào cơ sở dữ liệu":
        # Đường dẫn tới thư mục để trích xuất
        ...
        folder_path = paths[0]  # Use the first path as default for backward compatibility
        ...
        if st.button("Bắt đầu trích xuất và lưu vào CSDL"):
            with st.spinner("Đang trích xuất đặc trưng từ tất cả file âm thanh và lưu vào cơ sở dữ liệu..."):
                ...
                if not df_results.empty:
                    ...
                    # Tạo file CSV kết quả để download
        ...
        if uploaded_file is not None:
            ...
            # Lưu file và trích xuất đặc trưng
